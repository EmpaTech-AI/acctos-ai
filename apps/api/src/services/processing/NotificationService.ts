...
export function notifyParserError(alert: ParserErrorAlert): void {
    // If the emailSubject looks like a list of filenames (e.g. auto-generated by the sender's
    // email client from attachment names), fall back to the batch label so the notification
    // subject reads "batch (12 files)" rather than a long filename list.
    const looksLikeFilenames = (s: string) => /\.pdf|\.xlsx?/i.test(s);
    const clientName = (alert.emailSubject && !looksLikeFilenames(alert.emailSubject))
        ? alert.emailSubject
        : alert.label;
