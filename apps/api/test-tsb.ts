// Quick parser test: reads a TSB PDF, runs full pipeline, saves Excel output.
...
if (!filePath) { console.error('Usage: npx tsx test-tsb.ts <path-to-pdf>'); process.exit(1); }
...
const { transactions, statementTotals, ascending } = parseTsb(combined);
